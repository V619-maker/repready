...
# Last updated: July 26, 2026 (API auth fix)
...
| Sprint 14 — API auth hardening | ✅ Complete | Added Clerk server-side auth to the 4 endpoints in `app/api/[[...path]]/route.js` that previously trusted client-supplied identity (Known Issue #5) — only file touched. New `getAuthedEmail()` helper calls `auth()` then `currentUser()` from `@clerk/nextjs/server` (already covered by `middleware.js`'s existing `/(api\|trpc)(.*)` matcher — no middleware changes needed). `GET /sessions`, `GET /dashboard`, `GET /benchmark` now 401 without a session and derive identity/org server-side, ignoring any client-supplied `email`/`orgId`; `POST /rep-memory` 401s without a session and 403s if `body.userEmail` doesn't match it (403 not silent-override, since this call is billed). `POST`/`DELETE /api/sessions` intentionally left unchanged — not in the requested scope, flagged as a follow-up (Known Issue 5a). Verified via curl with no auth cookie that all 4 endpoints return 401 (including attempts to spoof identity via query param/body) and that unrelated endpoints (`/api/personas`, `POST /api/sessions`) are unaffected. Could not fully exercise the authenticated-success path end-to-end in this sandbox (no real Clerk session obtainable with dummy dev keys) — reviewed the query logic by hand instead: for a legitimate signed-in user, the server-derived email/orgId is byte-identical to what the client always sent (same Clerk user object, same `email.split('@')[1]` derivation as `app/dashboard/page.js` already used), so no query-shape change for real users. Two new issues found during this audit and documented rather than silently fixed: dashboard has no rep/manager role distinction (5b), and real sessions are saved with `orgId: null` because `app/deck/page.js` never sends one, so `/api/dashboard`'s org-scoped query may return empty for real orgs independent of this fix (5c). `npm run build` clean. |
...
5c. **Discovered while fixing #5 — sessions are saved with `orgId: null`.** `POST /api/sessions` sets `orgId: body.orgId || null`, but `app/deck/page.js`'s session-save call never sends `orgId` in the body at all — so every real session saved through the actual product journey has `orgId: null` in MongoDB. Since `/api/dashboard` queries `sessions` by `orgId`, this means the dashboard's org-level aggregates likely return empty for real orgs today, independent of the auth fix (the auth fix changes *where* `orgId` comes from for the query, not this underlying data gap). Needs its own fix: either send the rep's email-domain `orgId` when saving a session, or derive it server-side in `POST /api/sessions` the same way it's now derived in `GET /api/dashboard`.
