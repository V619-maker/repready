...
| Sprint 20 — Swap placeholder ElevenLabs agent IDs for Priya and Rakesh (branch `fix/persona-agent-ids`) | ⏳ Committed, not yet merged | Real ElevenLabs agents were created for both personas outside this repo. `app/deck/page.js`'s `PRIYA_ID`/`RAKESH_ID` placeholder string values replaced with the real IDs (Priya `agent_5701kzmwc61jfq59zjnvrvb1pxmt`, Rakesh `agent_4901m021sybbenebrf04zntdad30`), and the now-stale PLACEHOLDER warning comments tied to those two lines removed — nothing else in the file touched, verified via diff before committing. That branch's own copy of `REPREADY_CONTEXT.md` updates the ElevenLabs Agents table and the "not yet callable" note to match; this table (on `main`, pre-merge) still shows the old TBD placeholders and callability warning as of this writing, since the fix hasn't merged yet — deliberately not duplicated here to avoid two pending branches racing to edit the same paragraph. Role/company/first-message details for both personas are still undocumented — separate, still-open gap. `npm run build` clean. |
| Sprint 21 — Sharpen hero and pricing copy to lead with defensible differentiators (branch `copy/differentiation-messaging`) | ⏳ Committed, not yet merged | Copy-only change to `app/page.js`. Hero subheadline now states the hostility-escalation mechanic explicitly instead of burying it — "hostile AI buyers who get harder as your team improves" — since that's the single most ownable claim in the product. Pricing subheadline swaps a vague "unlike US competitors" line for a factual, specific one: no demo-gating, since most direct India competitors require a call before showing price, not just US ones. The demo-comparison section already had a line stating the difficulty-escalation mechanic concretely ("Richard gets harder between sessions... Difficulty is earned, not set."), so no line was added there — a second mention would have been redundant. No layout, structural, or visual-theme changes; independent of the (also not-yet-merged at the time) light-theme branch, which hadn't touched this copy. `npm run build` clean; dev-render screenshots confirmed both lines wrap cleanly with no overflow. |
