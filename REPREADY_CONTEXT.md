...
# Last updated: September 6, 2026 (Sprint 31 — clickjacking header fix; Sprint 30 — vendored
# compound-engineering-plugin skills — landed the same day. See those entries for detail.
...
0f. **Clickjacking half fixed in Sprint 31; CORS half still open.** ~~`X-Frame-Options: ALLOWALL` + CSP `frame-ancestors *` set for every route~~ — ✅ Fixed (Sprint 31, `fix/clickjacking-frame-headers`): `next.config.js` now sets `X-Frame-Options: DENY` and `frame-ancestors 'none'` site-wide. Checked first for a legitimate framing need (no iframe embeds of any repready page exist anywhere in the codebase) before picking the strict option over `SAMEORIGIN`/`'self'`. **Still open:** `next.config.js`'s `Access-Control-Allow-Origin: *` (default, overridable via `CORS_ORIGINS`) is paired with `Access-Control-Allow-Credentials: true` in the API layer's `handleCORS()` (`app/api/[[...path]]/route.js`) — a contradictory pairing browsers reject for credentialed requests in practice, and a sign nobody has configured CORS deliberately. Confirmed still present; fixing it (a real allowlist echoing the request origin, not `*`) was out of scope for this task.
