...
# Last updated: July 26, 2026 (session orgId fix)
...
**Note:** `orgId` is populated as of Sprint 14 (`userEmail.split('@')[1]`, set in `app/deck/page.js`'s `handleTerminate`). Sessions saved before Sprint 14 have `orgId: null` — any org-level query needs to handle that gap for historical data.
...
| Sprint 14 — Populate `orgId` on session save | ✅ Complete | One-line fix in `app/deck/page.js`'s `handleTerminate` (only file touched): the `POST /api/sessions` body built at line ~313 never included `orgId`, so every real session saved through the actual product journey had `orgId: null` in MongoDB — meaning any future `/api/dashboard` org-scoped query would return empty for real orgs regardless of how `orgId` is supplied. Added `orgId: userEmail ? userEmail.split('@')[1] : null` to that body object, matching the same email-domain derivation `app/dashboard/page.js` already uses client-side. Verified via `git diff` that this is a single-line addition (plus its trailing comma) with nothing else in the surrounding function touched. `npm run build` clean. |
