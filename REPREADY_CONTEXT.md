...
# Last updated: September 2, 2026 (full codebase audit — see Sprint 27 entry; this doc had
# drifted significantly behind merged code since the Aug 11 update, see that entry for detail)
...
- `app/demo/page.js` — ⚠️ **not part of the user journey, undocumented until the Sprint 27 audit found it.** Renders `components/RepReadyCoach.jsx`, a text-mode chat simulator — violates the voice-only constraint below and is currently broken (calls `/api/coach` with a request shape that endpoint doesn't accept; every call silently fails to a hardcoded fallback). Not linked from any nav, but listed in `sitemap.js` and not blocked in `robots.js`, so it's publicly indexable. See Known Issue 0a. Needs a product decision (delete vs. fix) before anyone builds on it.
...
0a. **CRITICAL — `/demo` is a fully-built TEXT-MODE chat page, violating the "voice-only" constraint, and it's broken.** `app/demo/page.js` renders `components/RepReadyCoach.jsx`, a complete text chat simulator ("CFO Pushback" scenario) that calls `/api/coach` with a `{messages, system, max_tokens}` shape the standalone `app/api/coach/route.js` doesn't accept (it wants `{transcript}`) — every real call 400s, silently caught, and the UI always shows the same hardcoded fallback response/debrief, forever. Session history is stored only in `localStorage` (`hooks/useRepMemory.js`), never MongoDB. It's linked nowhere in the app's own nav, but **is** listed in `app/sitemap.js` and not disallowed in `robots.js`, so it's publicly indexable — a broken, off-brand, unauthenticated page search engines can find. Needs a decision: delete it, or fix + bring in-policy (voice-only rule says the latter isn't really an option without a product decision first).

0b. **CRITICAL — `POST /api/sessions` lets anyone forge a fake session for any rep, including a fabricated top score.** Not just an auth gap (see 5a below) — `app/deck/page.js` computes `finalScore`/`qualificationStatus`/`grade`/`dimensions`/etc. **client-side** and sends them verbatim; the API stores whatever it's given with zero recomputation. Anyone who can see the request shape (trivial — it's client JS) can `POST` directly with `{userEmail: "victim@co.com", finalScore: 100, qualificationStatus: "Elite", ...}` and poison that rep's stats, the org dashboard, and the leaderboard. Needs either server-side score recomputation/verification or, at minimum, auth + a plausibility check.
...
0f. **MEDIUM — Wide-open CORS + framing headers, site-wide.** `next.config.js` sets `Access-Control-Allow-Origin: *` (default, overridable via undocumented-until-now `CORS_ORIGINS`) together with `Access-Control-Allow-Credentials: true` in the API layer — a contradictory pairing browsers will reject in practice, but signals nobody has actually configured CORS deliberately. Separately, `X-Frame-Options: ALLOWALL` + CSP `frame-ancestors *` is set for **every route**, meaning the entire app (including `/deck`, `/dashboard`) can be framed by any origin — a clickjacking exposure. Needs a deliberate CORS allowlist + frame policy, not the current defaults.
...
0k. **LOW — `/my-stats` and `/dashboard` aren't in `middleware.js`'s protected-route matcher** (only `/deck(.*)` and `/coach(.*)` are). Their underlying APIs are auth-gated so no data leaks, but the page shells themselves load for signed-out visitors, inconsistent with the documented "gated" user journey.
...
5a. **Still open** — `DELETE /api/sessions?email=xxx` remains fully unauthenticated; `POST /api/sessions` still trusts client-supplied `body.userEmail`/`body.orgId`/scores unverified. See new finding 0b above for the fuller exploit picture (score forgery, not just deletion).

5b. ~~`/api/dashboard` has no rep-vs-manager role check~~ — ✅ Fixed (Sprint 26, `feature/role-tier-gating`), **not caught by the last doc update**. `getAuthedUser()` now reads `role` from Clerk `publicMetadata` (defaults to `'rep'`) and `/api/dashboard` scopes its query to the caller's own sessions unless `role === 'manager'`. Practically no-op today since nobody has `role: 'manager'` set in Clerk yet, but the permission model exists and works — this item should have been closed weeks ago.
