...
# Last updated: September 6, 2026 (Sprint 32 — CORS/credentials contradiction fix; Sprint 30 —
# vendored compound-engineering-plugin skills — landed the same day. See those entries for
# detail. Prior major update: September 2, 2026 full codebase audit, Sprint 27)
...
0f. **CORS half fixed in this task; framing half still open on a separate, not-yet-merged PR (#29).** ~~`Access-Control-Allow-Origin: *` paired with `Access-Control-Allow-Credentials: true`~~ — ✅ Fixed: `app/api/[[...path]]/route.js` now resolves the origin dynamically per request via `applyCorsOriginPolicy()`, checked against an allowlist (`CORS_ORIGINS`, comma-separated; defaults to `https://repready.site`) — an allowed origin gets echoed back with credentials, anything else gets no CORS headers at all. The wrapper runs once, after `handleRouteInternal()` returns, so it didn't require touching the ~40 existing `handleCORS()` call sites inside route branches (those still set the old static value, which this wrapper then overwrites — harmless, if slightly redundant; a future cleanup could remove them). `next.config.js`'s static site-wide `Access-Control-Allow-Origin`/`-Methods`/`-Headers` lines were removed (a `headers()` config can't reflect a real per-request Origin, so they were always going to be wrong for this). **Still open, separate PR:** `X-Frame-Options: ALLOWALL` + CSP `frame-ancestors *` — see PR #29 (`fix/clickjacking-frame-headers`), not yet merged as of this task.
