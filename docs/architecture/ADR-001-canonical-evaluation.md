...
2. **Shadow write:** `/api/boardroom` creates a score result after successful scoring and additively returns `scoreResultId`. A shadow-write failure must not cost the rep their existing debrief.
