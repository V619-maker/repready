import './globals.css';
import Link from 'next/link'; // Added Next.js Link for routing

export const metadata = {
  title: 'RepReady // Simulation Engine',
      ...
      <body className="font-sans antialiased selection:bg-cyan-400/30 selection:text-cyan-400 relative">
        ...
        <header className="fixed top-0 w-full z-50 flex justify-between items-center px-6 h-16 bg-black/60 backdrop-blur-xl border-b border-white/10 shadow-[0_0_15px_rgba(34,211,238,0.1)]">
          <div className="flex items-center gap-3">
            ...
            {/* Rebranded to RepReady */}
            <span className="font-headline font-bold text-cyan-400 tracking-tighter text-xl uppercase">RepReady</span>
          </div>
        ...
        <aside className="fixed left-4 top-20 bottom-4 w-20 rounded-xl bg-[#0E0E0E]/80 backdrop-blur-3xl border border-white/10 shadow-2xl z-40 hidden lg:flex flex-col items-center py-8 gap-8 font-mono text-xs tracking-tighter">
          ...
          <nav className="flex flex-col gap-6 w-full items-center">
            ...
            {/* Dashboard Link */}
            <Link href="/" className="bg-cyan-400/10 text-cyan-400 border-r-2 border-cyan-400 w-full py-4 flex flex-col items-center gap-1">
              <span className="material-symbols-outlined" style={{fontVariationSettings: "'FILL' 1"}}>grid_view</span>
              <span>DASH</span>
            </Link>
            
            {/* Coach Link (Now routes to your existing AI Coach brain) */}
            <Link href="/coach" className="text-zinc-500 hover:text-cyan-400 hover:bg-white/5 w-full py-4 flex flex-col items-center gap-1 transition-all">
